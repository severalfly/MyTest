					...
					if (t && t.length > 0) {
						...
						for (var q = 0; q < t.length; q++) {
							...
							if (q == 0) {
								...
								$("#" + $.stopStation.defaultOptions.showTitleId).html(
										'<span class="item1">' + l.station_train_code + '次      </span><span class="item2">' + l.start_station_name + "<em>--></em>" + l.end_station_name + '</span><span class="item3">' + n + '</span><span class="item4">' + c + "</span>").find("span").css("color",
										"#333")
							}
							var m = "";
							if (!l.isEnabled) {
...
(function() {
	...
	var bn = function(cu) {
		...
		this.init = function() {
			...
			cw.css(cu, cp.x + "px").css({
				top : cp.y + "px"
			}).on("mouseenter", cs.stop).on("mouseleave", cs.resize).children("a.close").on("click", cs.hidden);
			$("body").append(cw);
			co = cp.x;
			ct = cp.y;
			cs.resize()
		...
		this.resize = function() {
			cr.dx = ($(window).width() - $(".content").width()) / 2 - cw.width();
			cr.dy = ($(window).height()) - cw.height();
			if (cr.dx <= (cp.x + Math.abs(cn.x)) || cr.dy <= (cp.y + Math.abs(cn.y))) {
				cs.stop()
			} else {
				cs.alive()
			}
	...
	function G() {
		setTimeout(function() {
			if (!frameComplete) {
				...
				co.remove();
				$("#myfix_yh").css("background", "url(" + cp + ") no-repeat");
	...
	function aA(cn) {
			...
			if (cu > 0 && aO) {
				...
				if (!$("#autoSubmit").is(":checked")) {
					...
					if (ccSelected.length > 0 || rqChecked.length > 0 || xbChecked.length > 0) {
						...
						if (train_tour_flag == "fc") {
							var cv = "成功查到" + $("#back_train_date").val() + "的" + bJ[0]["queryLeftNewDTO"]["station_train_code"] + "次"
						} else {
							var cv = "成功查到" + $("#train_date").val() + "的" + bJ[0]["queryLeftNewDTO"]["station_train_code"] + "次"
						}
						if (cu == 1) {
							cv = cv + "车。"
						} else {
				...
				$("#filterTicDiv").html("<strong>离下次刷新时间<font class='colorA' style='font-size: 16px;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + ct + "</font>秒<strong>");
		...
		for (var co = 0; co < cn.length; co++) {
			...
			cs.push("<div><a  " + cr + ' title="点击查看停靠站信息" href="javascript:" id="');
			cs.push(cn[co].queryLeftNewDTO.train_no);
			cs.push("_");
			cs.push(cn[co].queryLeftNewDTO.from_station_telecode);
			cs.push("_");
			cs.push(cn[co].queryLeftNewDTO.to_station_telecode);
			if ("1" == cn[co].queryLeftNewDTO.controlled_train_flag || "2" == cn[co].queryLeftNewDTO.controlled_train_flag) {
			...
			if ("1" == cn[co].queryLeftNewDTO.controlled_train_flag || "2" == cn[co].queryLeftNewDTO.controlled_train_flag) {
				cs.push('" class="lookup"><span style="display:none;">查看票价</span><b style="display:none;" title="查看票价"></b></span>')
			} else {
				cs.push('" class="lookup" onclick="showTicketPrice(this)"><span style="display:none;">查看票价</span><b title="查看票价"></b></span>')
			}
			cs.push("</div>");
			cs.push('<div class="cdz">');
			if (cn[co].queryLeftNewDTO.from_station_telecode != null && cn[co].queryLeftNewDTO.from_station_telecode == cn[co].queryLeftNewDTO.start_station_telecode) {
			...
			if (cn[co].queryLeftNewDTO.to_station_telecode != null && cn[co].queryLeftNewDTO.to_station_telecode == cn[co].queryLeftNewDTO.end_station_telecode) {
			...
			if ("1" == cn[co].queryLeftNewDTO.controlled_train_flag || "2" == cn[co].queryLeftNewDTO.controlled_train_flag) {
				...
				cs.push("</strong>");
				cs.push("<span>");
				cs.push(changeArriveDate(cn[co].queryLeftNewDTO.start_time, cn[co].queryLeftNewDTO.lishi));
				cs.push("到达</span>")
			}
			cs.push("</div>");
			cs.push("</div>");
			cs.push("</td>");
			if (cn[co].queryLeftNewDTO.swz_num && cn[co].queryLeftNewDTO.swz_num != "--" && cn[co].queryLeftNewDTO.swz_num != 0 && cn[co].queryLeftNewDTO.swz_num != "无") {
				b5(cs, cn[co].queryLeftNewDTO.swz_num, "SWZ_", cn[co].queryLeftNewDTO.train_no, cn[co].queryLeftNewDTO.yp_ex, "91", cn[co].queryLeftNewDTO.controlled_train_flag)
			} else {
				if (cn[co].queryLeftNewDTO.tz_num && cn[co].queryLeftNewDTO.tz_num != "--" && cn[co].queryLeftNewDTO.tz_num != 0 && cn[co].queryLeftNewDTO.tz_num != "无") {
					b5(cs, cn[co].queryLeftNewDTO.tz_num, "TZ_", cn[co].queryLeftNewDTO.train_no, cn[co].queryLeftNewDTO.yp_ex, "P1", cn[co].queryLeftNewDTO.controlled_train_flag)
				} else {
					if (cn[co].queryLeftNewDTO.swz_num && cn[co].queryLeftNewDTO.swz_num == "无") {
						b5(cs, cn[co].queryLeftNewDTO.swz_num, "SWZ_", cn[co].queryLeftNewDTO.train_no, cn[co].queryLeftNewDTO.yp_ex, "91", cn[co].queryLeftNewDTO.controlled_train_flag)
					} else {
						b5(cs, cn[co].queryLeftNewDTO.tz_num, "TZ_", cn[co].queryLeftNewDTO.train_no, cn[co].queryLeftNewDTO.yp_ex, "P1", cn[co].queryLeftNewDTO.controlled_train_flag)
					}
			...
			b5(cs, cn[co].queryLeftNewDTO.zy_num, "ZY_", cn[co].queryLeftNewDTO.train_no, cn[co].queryLeftNewDTO.yp_ex, "M1", cn[co].queryLeftNewDTO.controlled_train_flag);
			b5(cs, cn[co].queryLeftNewDTO.ze_num, "ZE_", cn[co].queryLeftNewDTO.train_no, cn[co].queryLeftNewDTO.yp_ex, "O1", cn[co].queryLeftNewDTO.controlled_train_flag);
			b5(cs, cn[co].queryLeftNewDTO.gr_num, "GR_", cn[co].queryLeftNewDTO.train_no, cn[co].queryLeftNewDTO.yp_ex, "61", cn[co].queryLeftNewDTO.controlled_train_flag);
			b5(cs, cn[co].queryLeftNewDTO.rw_num, "RW_", cn[co].queryLeftNewDTO.train_no, cn[co].queryLeftNewDTO.yp_ex, "41", cn[co].queryLeftNewDTO.controlled_train_flag);
			b5(cs, cn[co].queryLeftNewDTO.srrb_num, "SRRB_", cn[co].queryLeftNewDTO.train_no, cn[co].queryLeftNewDTO.yp_ex, "F1", cn[co].queryLeftNewDTO.controlled_train_flag);
			b5(cs, cn[co].queryLeftNewDTO.yw_num, "YW_", cn[co].queryLeftNewDTO.train_no, cn[co].queryLeftNewDTO.yp_ex, "31", cn[co].queryLeftNewDTO.controlled_train_flag);
			b5(cs, cn[co].queryLeftNewDTO.rz_num, "RZ_", cn[co].queryLeftNewDTO.train_no, cn[co].queryLeftNewDTO.yp_ex, "21", cn[co].queryLeftNewDTO.controlled_train_flag);
			b5(cs, cn[co].queryLeftNewDTO.yz_num, "YZ_", cn[co].queryLeftNewDTO.train_no, cn[co].queryLeftNewDTO.yp_ex, "11", cn[co].queryLeftNewDTO.controlled_train_flag);
			b5(cs, cn[co].queryLeftNewDTO.wz_num, "WZ_", cn[co].queryLeftNewDTO.train_no, cn[co].queryLeftNewDTO.yp_ex, "W1", cn[co].queryLeftNewDTO.controlled_train_flag);
			b5(cs, cn[co].queryLeftNewDTO.qt_num, "QT_", cn[co].queryLeftNewDTO.train_no, cn[co].queryLeftNewDTO.yp_ex, "", cn[co].queryLeftNewDTO.controlled_train_flag);
			if ("Y" == cn[co].queryLeftNewDTO.canWebBuy) {
				cs.push(' <td align="center" width="80" class="no-br"><a href="javascript:" class="btn72" onclick="checkG1234(\'');
			...
			cs.push("</tr>");
			cs.push('<tr datatran="' + cn[co].queryLeftNewDTO.station_train_code + '" id="price_');
			cs.push(cn[co].queryLeftNewDTO.train_no);
	...
	function ae(ct, cn) {
		...
		if (hainan_limit_start_traindate && C(cq) >= C(hainan_limit_start_traindate)) {
			if (hainan_limit_from_telcode && hainan_limit_to_telcode) {
				for (var co = 0, cs = cn.length; co < cs; co++) {
					...
					if (hainan_limit_from_telcode.indexOf(cu) > -1 && hainan_limit_to_telcode.indexOf(cr) > -1 && cp.indexOf("起售") > -1) {
						return true
					}
				}
			}
	...
	function b5(cr, co, cw, cs, cp, cv, cq) {
		...
		if ("1" == cq || "2" == cq) {
			cr.push(' <td width="46" align="center" style="cursor: pointer;"  id="');
			cr.push(cw);
			cr.push(cs);
			cr.push('">');
			cr.push(co);
			...
			if ("有" == co) {
				if (cw == "SWZ_" || cw == "TZ_") {
					cr.push('<td width="46" align="center" class="yes" onclick="showTicketPrice(this)"　style="cursor: pointer;" id="');
					cr.push(cw);
					cr.push(cs);
					cr.push('">');
					if (ct) {
						cr.push('<div class="sale" title="本席别票价打折">' + co + '<span class="i-mark">折</span></div>')
					} else {
						cr.push(co)
					}
					cr.push("</td>")
					...
					if (cw == "ZY_" || cw == "ZE_") {
						cr.push('<td width="46" align="center" class="yes" style="cursor: pointer;" onclick="showTicketPrice(this)" id="');
						cr.push(cw);
						cr.push(cs);
						cr.push('">');
						if (ct) {
						...
						cr.push('<td width="46" align="center" style="cursor: pointer;" class="yes" onclick="showTicketPrice(this)" id="');
						cr.push(cw);
						cr.push(cs);
						cr.push('">');
						if (ct) {
				...
				if (co == "无" || isNum(co) >= 0) {
					...
					if (cw == "SWZ_" || cw == "TZ_" || cw == "ZY_" || cw == "ZE_") {
						cr.push('<td width="46" align="center" style="cursor: pointer;" ' + cn + ' onclick="showTicketPrice(this)" id="');
						cr.push(cw);
						cr.push(cs);
						cr.push('">');
						cr.push("<div>");
					...
					cr.push(' <td width="46" align="center" style="cursor: pointer;" onclick="showTicketPrice(this)"  id="');
					cr.push(cw);
					cr.push(cs);
					cr.push('">');
					cr.push(co);
	...
	function k(co, cn) {
		...
		for (i = 0; i < co.length; i++) {
			if (co[i][0] == cn) {
				co[i][1] = $("#".concat($("#".concat(cn)).attr("for"))).is(":checked");
				ishaveCheckId = true
			}
		}
		if (!ishaveCheckId) {
	...
	function C(co) {
		var cn = new Date();
		var cp = co.split("-");
		cn.setFullYear(parseInt(cp[0]), parseInt(cp[1] - 1, 10), parseInt(cp[2], 10));
		if (cp.length >= 6) {
	...
	Date.prototype.format = function(co) {
		...
		if (/(y+)/.test(co)) {
			co = co.replace(RegExp.$1, (this.getFullYear() + "").substr(4 - RegExp.$1.length))
		}
		for ( var cn in cp) {
			if (new RegExp("(" + cn + ")").test(co)) {
				co = co.replace(RegExp.$1, RegExp.$1.length == 1 ? cp[cn] : ("00" + cp[cn]).substr(("" + cp[cn]).length))
			}
		}
		return co
	};
	...
	function ar(cp) {
		for (var cn = 1; cn <= 20; cn++) {
			var co = $("#date_range>ul>li:nth-child(" + cn + ")").children("span:first-child").text();
			co = "2013-" + co;
			if (cp == co) {
				$("#date_range>ul>li").removeClass("sel");
				$("#date_range>ul>li").removeAttr("alt");
				$("#date_range>ul>li:nth-child(" + cn + ")").addClass("sel");
				$("#date_range>ul>li:nth-child(" + cn + ")").attr("alt", "show");
				$("#date_range>ul>li:nth-child(20)").addClass("end");
				$("#date_range>ul>li:nth-child(" + cn + ")").children("span:first-child").removeClass();
				$("#date_range>ul>li:nth-child(" + cn + ")").children("span:last-child").removeClass();
				$("#date_range>ul>li:nth-child(" + cn + ")").children("span:first-child").addClass("hide");
				$("#date_range>ul>li:nth-child(1)").children().addClass("first");
				bR = $("#date_range>ul>li:nth-child(" + cn + ")").children("span:first-child").text();
				break
			}
		}
	}
	...
	function bT() {
		$("#change_station").bind("click", function() {
			...
			if ((cr != "" && cr != "简拼/全拼/汉字") && (cs != "" && cs != "简拼/全拼/汉字")) {
	...
	function bK() {
		$("#query_ticket").unbind("click").click(function(cr) {
			$("#sel-buyer").hide();
			$("#sel-seat").hide();
			$("#sel-date").hide();
			...
			var co = train_tour_flag == "fc" ? $.trim($("#back_train_date").val()) : $.trim($("#train_date").val());
			var cn = bI(co, cs);
			if (!cn) {
				return
			}
	...
	function b2(cq, cs) {
		...
		for (var co = 0; co < cq.length; co++) {
			...
			cr.wz_num = cn[26] ? cn[26] : "--"; // 无座
			cr.yb_num = cn[27] ? cn[27] : "--";
			cr.yw_num = cn[28] ? cn[28] : "--";
			cr.yz_num = cn[29] ? cn[29] : "--";
			cr.ze_num = cn[30] ? cn[30] : "--"; // 二等座
			cr.zy_num = cn[31] ? cn[31] : "--"; // 一等座
			cr.swz_num = cn[32] ? cn[32] : "--"; // 商务座
	...
	function aj(cn) {
		...
		$.ajax({
			...
			error : function(cp, cr, cq) {
				...
				if ("timeout" == cr || "No Transport" == cr || "abort" == cr) {
					if ($("#auto_query").is(":checked")) {
						aj(cn)
					}
				}
			...
			success : function(cr) {
				dhtmlx.modalbox.hide(co);
				if (cr.status) {
					if (cr.data == null || cr.data.length == 0 || (cr.data.result && cr.data.result.length == 0)) {
						$("#sear-sel").hide();
						$("#sear-result").hide();
						$("#t-list").hide();
						$("#_lc_link_foot").hide();
					...
					for (var cs = 0; cs < cr.data.length; cs++) {
						trainListForIE.push(cr.data[cs].queryLeftNewDTO.station_train_code + "(" + cr.data[cs].queryLeftNewDTO.start_time + "--" + cr.data[cs].queryLeftNewDTO.arrive_time + ")")
					}
					...
					if (train_tour_flag != null && train_tour_flag == "fc") {
						var cy = "<strong>".concat($("#fromStationText").val()).concat(" --> ").concat($("#toStationText").val()).concat("（").concat(aD($("#back_train_date").val())).concat('）</strong>共计<strong id="trainum">').concat(cr.data.length).concat("</strong>个车次");
						if (bV(cr.data)) {
							cp = "<p class='ad-gt' id='DW_SHOW_STR' data='1'>高铁动卧 夕发朝至 风雨无阻 省时省钱</p>"
						} else {
							if (hainan_limit_msg && ae(cn, cr.data)) {
								cu = "<p class='ad-gt' id='hainan_limit_msg'  style='font-size:13px;background:#fff6f6 left center no-repeat;font-weight:bold'>" + hainan_limit_msg + "</p>"
							}
						}
						if ($("#auto_query").is(":checked")) {
							var cw = "";
							...
							cy = cy.concat(cw + "<input type='checkbox' class='check' id='filterTic' /><div id='filterTicDiv' style='display:inline'><strong><label for='filterTic' style='cursor: pointer;'>仅查看刷到车次</label></strong></div>")
						}
						...
						var cy = "<strong>".concat($("#fromStationText").val()).concat(" --> ").concat($("#toStationText").val()).concat("（").concat(aD($("#train_date").val())).concat('）</strong>共计<strong id="trainum">').concat(cr.data.length).concat("</strong>个车次");
						if (bV(cr.data)) {
							cp = "<p class='ad-gt' id='DW_SHOW_STR' data='1'>高铁动卧 夕发朝至 风雨无阻 省时省钱</p>"
						} else {
							if (hainan_limit_msg && ae(cn, cr.data)) {
					...
					if (!$("#lc_msg")[0] && cu == "" && cp == "") {
						var cx = '<p id="lc_msg">您可使用<a  href="' + ctx + 'lcQuery/init">接续换乘</a>功能，查询途中换乘一次的部分列车余票情况。</p>';
			...
			error : function(cp, cr, cq) {
				...
				if (cp.status == 403) {
					...
					if (cp.responseText == "0" || cp.responseText == "2" || cp.responseText == "3" || cp.responseText == "4" || cp.responseText == "7") {
						cj("非法请求！");
						return
					} else {
						if (cp.responseText == "1" || cp.responseText == "5" || cp.responseText == "6") {
							cj("您的操作频率过快，请稍后再试！");
							return
						} else {
							cj("查询失败，请稍后再试！");
	...
	function ab() {
		...
		var co;
		$("#date_range>ul>li").each(function(cs) {
			var cq = fullDateArr[cs];
			var cp = new Date(Date.parse(cq.replace(/-/g, "/")));
			var cr = $("#date_range>ul>li:nth-child(" + (cs + 1) + ")>span[class=hide]").text().substring(0, 5) + bi(cp);
			$("#date_range>ul>li:nth-child(" + (cs + 1) + ")>span[class=hide]").text(cr);
	...
	function bg() {
		...
		$("#date_range>ul>li").bind("mouseout", function() {
			$("#date_range>ul>li").each(function(cn) {
	...
	function Q(cp, co) {
		...
		for (var cn = 0; cn < co.length; cn++) {
			...
			if (co[cn] == "QT") {
				if (cp.queryLeftNewDTO.station_train_code.substring(0, 1) != "G" && cp.queryLeftNewDTO.station_train_code.substring(0, 1) != "D" && cp.queryLeftNewDTO.station_train_code.substring(0, 1) != "C" && cp.queryLeftNewDTO.station_train_code.substring(0, 1) != "T"
						&& cp.queryLeftNewDTO.station_train_code.substring(0, 1) != "K" && cp.queryLeftNewDTO.station_train_code.substring(0, 1) != "Z") {
					return true
				}
			}
			if (co[cn] == "G") {
				if (cp.queryLeftNewDTO.station_train_code.substring(0, 1) == "C" || cp.queryLeftNewDTO.station_train_code.substring(0, 1) == "G") {
					return true
				}
			}
		}
	...
	function aQ(cp, cn) {
		...
		for (var co = 0; co < cn.length; co++) {
			if (cn[co] == "SWZ") {
				if (cp.queryLeftNewDTO.swz_num != "--" && cp.queryLeftNewDTO.swz_num != "无") {
					aC.push("SWZ");
					return true
				}
			}
			if (cn[co] == "TZ") {
				if (cp.queryLeftNewDTO.tz_num != "--" && cp.queryLeftNewDTO.tz_num != "无") {
					aC.push("TZ");
					return true
				}
			}
			if (cn[co] == "ZY") {
				if (cp.queryLeftNewDTO.zy_num != "--" && cp.queryLeftNewDTO.zy_num != "无") {
					aC.push("ZY");
					return true
				}
			}
			if (cn[co] == "ZE") {
				if (cp.queryLeftNewDTO.ze_num != "--" && cp.queryLeftNewDTO.ze_num != "无") {
					aC.push("ZE");
					return true
				}
			}
			if (cn[co] == "GR") {
				if (cp.queryLeftNewDTO.gr_num != "--" && cp.queryLeftNewDTO.gr_num != "无") {
					aC.push("GR");
					return true
				}
			}
			if (cn[co] == "RW") {
				if (cp.queryLeftNewDTO.rw_num != "--" && cp.queryLeftNewDTO.rw_num != "无") {
					aC.push("RW");
					return true
				}
			}
			if (cn[co] == "YW") {
				if (cp.queryLeftNewDTO.yw_num != "--" && cp.queryLeftNewDTO.yw_num != "无") {
					aC.push("YW");
					return true
				}
			}
			if (cn[co] == "RZ") {
				if (cp.queryLeftNewDTO.rz_num != "--" && cp.queryLeftNewDTO.rz_num != "无") {
					aC.push("RZ");
					return true
				}
			}
			if (cn[co] == "YZ") {
				if (cp.queryLeftNewDTO.yz_num != "--" && cp.queryLeftNewDTO.yz_num != "无") {
					aC.push("YZ");
					return true
				}
			}
			if (cn[co] == "SRRB") {
				if (cp.queryLeftNewDTO.srrb_num != "--" && cp.queryLeftNewDTO.srrb_num != "无") {
					aC.push("SRRB");
					return true
				}
			}
			if (cn[co] == "YYRW") {
				if (cp.queryLeftNewDTO.yyrw_num != "--" && cp.queryLeftNewDTO.yyrw_num != "无") {
					aC.push("YYRW");
					return true
				}
			}
			if (cn[co] == "WZ") {
				if (cp.queryLeftNewDTO.wz_num != "--" && cp.queryLeftNewDTO.wz_num != "无") {
					aC.push("WZ");
					return true
				}
			}
	...
	function w(co, cn) {
		...
		for (var cp = 0; cp < cn.length; cp++) {
			if (cn[cp].toLowerCase() == co.queryLeftNewDTO.station_train_code.toLowerCase()) {
				return true
			}
		}
		return false
	...
	function bc() {
		...
		var cp = a8;
		var ct = [];
		if (co.length > 0 || cu.length > 0 || filteredTrainArriveTime.length > 0 || bQ.length > 0 || cq.length > 0 || cs.length > 0 || ax.getComboText() != "" || $("#avail_ticket")[0].checked) {
	...
	function bF() {
		if (ischeckAutoSubmitCode) {
			$("#randCode2").on("keyup", function(cn) {
						...
						success : function(cp) {
							if (cp.data == "N") {
								...
								if (isAsync == ticket_submit_order.request_flag.isAsync) {
									if (K.queryLeftNewDTO.train_no.indexOf(co) > -1 && cr.indexOf(K.queryLeftNewDTO.from_station_telecode) > -1 && cq.indexOf(K.queryLeftNewDTO.to_station_telecode) > -1) {
										dhtmlx.createWin({
											winId : "confirmG1234",
				...
				if (bJ.length > 0) {
					...
					if (document.getElementById("autoSubmit").checked) {
						...
						for (var cx = 0; cx < passengerChecked.length; cx++) {
							...
							for (var cy = 0; cy < cr.length; cy++) {
								...
								if (cu.passenger_type != 2) {
									if (cu.passenger_name == co.passenger_name && cu.passenger_id_type_code == co.passenger_id_type_code && cu.passenger_id_no == co.passenger_id_no) {
						...
						if (cD[0] == 1 || cD[0] == 2) {
							...
							var cA = K.secretStr;
							m(cD);
							var cz = checkusermdId != undefined ? "&_json_att=" + encodeURIComponent(checkusermdId) : "";
							var cn = "";
							if ($("#dc").is(":checked")) {
								cn = "dc"
							} else {
							...
							if ("undefined" == typeof (submitForm)) {
								var cE = "secretStr=" + cA + "&train_date=" + $("#train_date").val() + "&tour_flag=" + cn + "&purpose_codes=" + cw + "&query_from_station_name=" + $("#fromStationText").val() + "&query_to_station_name=" + $("#toStationText").val() + "&" + cz
										+ "&cancel_flag=2&bed_level_order_num=000000000000000000000000000000&passengerTicketStr=" + getpassengerTicketsForAutoSubmit() + "&oldPassengerStr=" + getOldPassengersForAutoSubmit()
							} else {
								var cp = submitForm();
								var cv = cp.split(":::");
								var cC = cv[0].split(",-,")[0];
								...
								var cB = cv[1].split(",-,")[0];
								var cs = cv[1].split(",-,")[1];
								var cE = escape(cC) + "=" + escape(ct) + "&" + cB + "=" + cs + "&secretStr=" + cA + "&train_date=" + $("#train_date").val() + "&tour_flag=" + cn + "&purpose_codes=" + cw + "&query_from_station_name=" + $("#fromStationText").val() + "&query_to_station_name="
										+ $("#toStationText").val() + "&" + cz + "&cancel_flag=2&bed_level_order_num=000000000000000000000000000000&passengerTicketStr=" + getpassengerTicketsForAutoSubmit() + "&oldPassengerStr=" + getOldPassengersForAutoSubmit()
							}
							$.ajax({
								type : "post",
								...
								success : function(cF) {
									if (cF.status) {
										if (!cF.data.submitStatus) {
											if (cF.data.isRelogin) {
												...
												if (cF.data.isNoActive) {
													ac(cF.data.errMsg, true, "", true, "warn")
												} else {
													if (cF.data.checkSeatNum) {
														ac("很抱歉，无法提交您的订单!", true, "原因： " + cF.data.errMsg, true, "warn")
													} else {
														ac("车票信息不合法!", true, "原因： " + cF.data.errMsg, true, "warn")
													}
												}
											}
											return
										...
										if (cF.data.smokeStr != "" && cF.data.smokeStr.length > 0) {
											$("#dialog_smoker_msg").html(cF.data.smokeStr);
											dhtmlx.createWin({
												winId : "dialog_smoker",
	...
	function l(co, cp) {
		...
		if (co.data && undefined != co.data.result && typeof (co.data.result) != "undefined") {
	...
	function m(cz) {
		...
		for (var cv = 0; cv < ct.length; cv++) {
			...
			for (var cw = 0; cw < cx; cw++) {
				...
				tickets_info.push(new a9("sdAdd_" + am(), co, cp, cu, cq, passengerChecked[cs].passenger_name, passengerChecked[cs].passenger_id_type_code, passengerChecked[cs].passenger_id_type_name, passengerChecked[cs].passenger_id_no, passengerChecked[cs].mobile_no));
				cs++
			}
		}
	}
	function cg(co) {
		...
		if ("X" == canChooseBeds) {
			...
			cn = $("#autoSubCheckTicketInfoTemplate_chooseBeds").html().replace("<!--", "").replace("-->", "");
			$("#bed_show").show()
		} else {
			$("#bed_show").hide();
	...
	getpassengerTicketsForAutoSubmit = function() {
		...
		for (var ct = 0; ct < tickets_info.length; ct++) {
			...
			var cr = $("#autosubmit_check_ticketInfo").find("select[id^=ticketype_]");
			var cv = "0";
			if (cr && cr.length > 0) {
				var cn = tickets_info[ct].seat_type + "#" + tickets_info[ct].ticket_type + "#" + tickets_info[ct].name + "#" + tickets_info[ct].id_no;
			...
			var cx = cu + "," + cv + "," + tickets_info[ct].ticket_type + "," + tickets_info[ct].name + "," + tickets_info[ct].id_type + "," + tickets_info[ct].id_no + "," + (tickets_info[ct].phone_no == null ? "" : tickets_info[ct].phone_no) + ",N";
			co += cx + "_"
		}
	...
	getOldPassengersForAutoSubmit = function() {
		...
		for (var co = 0; co < passengerChecked.length; co++) {
			var cn = passengerChecked[co].passenger_name + "," + passengerChecked[co].passenger_id_type_code + "," + passengerChecked[co].passenger_id_no + "," + passengerChecked[co].passenger_type;
			cp += cn + "_"
		}
		return cp
	};
			...
			$.ajax({
				...
				success : function(cv) {
					if (cv.status) {
						...
						cs = "本次列车，" + (tickets_info[0].seat_type_name).split("（")[0] + "余票";
						if (parseInt(cw[0]) >= 0) {
						...
						if (cv.data.op_2 == "true") {
							...
							if (cv.data.countT > 0) {
								cs += '目前排队人数<font color="red">' + cv.data.countT + "</font>人，";
			...
			success : function(cn) {
				if (cn.status) {
					if (cn.data.submitStatus) {
						...
						ac("出票失败!", false, "原因： " + cn.data.errMsg + '<a  id="xg_close_win_id">点击修改</a>', true, "lose");
	...
	function an(cn, cp, co) {
		if (cp <= 5) {
			...
			if (cp > 30 * 60) {
				...
				ac("订单已经提交，最新预估等待时间" + co + "，请耐心等待。", false, "", false, "queue")
			}
		}
	}
	function r(cn, cp, co) {
	...
	function R() {
		...
		ac = function(cx, cu, cr, cq, ct) {
			var cw = '<div class="tit">' + (cu ? '<span class="colorC">' + cx + "</span>" : cx) + "</div>";
			var cs = "<P>" + cr + "</p>";
			var cv = cu ? '<p>请点击[<a href="' + ctx + 'queryOrder/init"><strong>我的12306</strong></a>]办理其他业务。您也可以点击[<a href="' + ctx + 'leftTicket/init"><strong>预订车票</strong></a>]，重新规划您的旅程。</p>' : '<P>查看订单处理情况，请点击“<a href="' + ctx + 'queryOrder/initNoComplete">未完成订单</a>”</P>';
			...
			$("#orderResultInfo_id").html(cw + (cr == "" || cr == null || cr == "undefined" || cr == undefined ? "" : cs) + cv);
		...
		function cn(cu) {
			...
			if ("checkticketinfo_id" == cu) {
				...
				if (cs.to_station_telecode == ticket_submit_order.special_areas.lso || cs.to_station_telecode == ticket_submit_order.special_areas.dao || cs.to_station_telecode == ticket_submit_order.special_areas.ado || cs.to_station_telecode == ticket_submit_order.special_areas.nqo
			...
			cr.setDimension($("#content_" + cu).width(), $("#content_" + cu).height() + 10);
		...
		function co() {
			for (var cr = 0; cr < limit_tickets.length; cr++) {
				var cq = limit_tickets[cr];
				if ((ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.fc || ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.gc) && cq.save_status != ""
						&& (cq.id_type == ticket_submit_order.passenger_card_type.passport || cq.id_type == ticket_submit_order.passenger_card_type.work || cq.id_type == ticket_submit_order.passenger_card_type.taiwan)) {
					return true
				} else {
	...
	function aD(cn) {
		...
		var cr = "日一二三四五六";
		return cq.concat(cp).concat("&nbsp;&nbsp;").concat("周").concat(cr.charAt(co.getDay()))
	}
	showTicketPrice = function(cr) {
		var cv = $(cr).parent("tr").children("td").children("div").children("div").children("span").attr("id");
		...
		if (cp && ($("#ticket_" + cs + "_train>span>span").text() == "查看票价")) {
			...
			$.ajax({
				...
				success : function(cx) {
					if (cx.status) {
						$("#ticket_" + cs + "_train>span>span").html("收起票价");
						$("#ticket_" + cs + "_train>span>b").addClass("open");
						$("#ticket_" + cs + "_train>span>b").attr("title", "收起票价");
						if (co == "--") {
							cx.data.MIN = ""
						}
						if (cn == "--") {
						...
						$("#price_" + cs).html($.render.priceTableTemplate(cx.data));
						$("#price_" + cs).show();
						if (ct && c(ct)) {
							$("#price_" + cs).find("td").eq(0).html('<a class="ad-tlist-hot" href="javascript:void(0);">移动宾馆 免费晚餐 快捷舒适 准时正点</a>')
						} else {
							$("#price_" + cs).find("td").eq(0).html("")
						}
						if (cx.data.PM != "--") {
							$("#sleeper-price_" + cs).mouseover(function() {
								$("#tp-list-price_" + cs).show()
							});
							$("#sleeper-price_" + cs).mouseout(function() {
								$("#tp-list-price_" + cs).hide()
							})
						}
					}
				}
	...
	checkHover = function(co, cn) {
		if (getEvent(co).type == "mouseover") {
			return !$.contains(cn, getEvent(co).relatedTarget || getEvent(co).fromElement) && !((getEvent(co).relatedTarget || getEvent(co).fromElement) === cn)
		} else {
			return !$.contains(cn, getEvent(co).relatedTarget || getEvent(co).toElement) && !((getEvent(co).relatedTarget || getEvent(co).toElement) === cn)
		}
	};
				...
				if (cp.data.flag) {
					...
					if (x == "0X00") {
						...
						for (i = (studentComPerArr.length - 1); i >= 0; i = i - 2) {
							if (C(studentComPerArr[i - 1]) <= C(cr) && C(studentComPerArr[i]) >= C(cr)) {
								cq = true;
								break
							}
						}
	...
	function S(cA, ct) {
		...
		if (train_tour_flag == "fc") {
			cn = "fc";
			var cq = ct.split(":");
			var cp = $("#back_train_date").val() + "-" + cq[0] + "-" + cq[1] + "-00";
			try {
				if (roundReferTime) {
		...
		if ("undefined" == typeof (submitForm)) {
			var cr = "secretStr=" + cA + "&train_date=" + $("#train_date").val() + "&back_train_date=" + $("#back_train_date").val() + "&tour_flag=" + cn + "&purpose_codes=" + ch() + "&query_from_station_name=" + $("#fromStationText").val() + "&query_to_station_name=" + $("#toStationText").val()
					+ "&" + cy
		} else {
			var co = submitForm();
			var cz = co.split(":::");
			...
			var cs = cz[1].split(",-,")[0];
			var cw = cz[1].split(",-,")[1];
			var cr = escape(cu) + "=" + escape(cx) + "&" + cs + "=" + cw + "&secretStr=" + cA + "&train_date=" + $("#train_date").val() + "&back_train_date=" + $("#back_train_date").val() + "&tour_flag=" + cn + "&purpose_codes=" + ch() + "&query_from_station_name=" + $("#fromStationText").val()
					+ "&query_to_station_name=" + $("#toStationText").val() + "&" + cy
		}
					...
					if (cp.test(cr) && cr.length > 4) {
						...
						if (cr.length < 2) {
							...
							if ($.inArray(cr, ccSelected) < 0) {
								var cn = "<span name='prior_train-span' class='sel-box w80'>" + cr + "<a class='close' href='javascript:' onclick='$.removeSel(this,\"" + cr + "\",4)'></a></span>";
	...
	function u(cB, cn) {
		...
		if (cA && cA.length > 0) {
			...
			for (var cq = 0; cq < cA.length; cq++) {
				var cv = cA[cq];
				var cr = cv.indexOf("(") > -1 ? cv.substring(0, cv.indexOf("(")) : cv;
				var cy = cq >= yxTrainPageSize * (cn) && cq < yxTrainPageSize * (cn + 1);
				if (cy) {
					if (cr.indexOf(cB) > -1) {
						...
						if (cx && cx.length > 0 && $.inArray(cr, cx) > -1) {
							cs += '<li style="width: 140px;" traincode=' + cr + ' name="yxtrainli" class="cur"><span style="font-size:15px;">' + ct + '<span style="color:red;">' + cB + "</span>" + cz + "</span>" + cp + "</li>"
						} else {
							cs += '<li style="width: 140px;" traincode=' + cr + ' name="yxtrainli"><span style="font-size:15px;">' + ct + '<span style="color:red;">' + cB + "</span>" + cz + "</span>" + cp + "</li>"
						}
					}
				}
			}
	...
	function aE(cv, cq) {
		var cr = "";
		cr += (cv == 0) ? "" : '<a href="javascript:void(0);" onclick="$.click_YX_page(' + (cv - 1) + ')" class="prev">上一页</a>';
		var cw = cv + 1;
		var cs = cq;
		var ct = 2;
		var cu = 5;
		...
		if (cs < cu) {
			for (var cp = 1; cp < cs + 1; cp++) {
				if (cw == cp) {
					cr += '<a href="javascript:void(0);" onclick="$.click_YX_page(' + (cp - 1) + ')" class="on">' + (cp) + "</a>"
				} else {
					cr += '<a href="javascript:void(0);" onclick="$.click_YX_page(' + (cp - 1) + ')">' + (cp) + "</a>"
				}
			}
		} else {
		...
		cr += (cv == cq - 1) ? "" : '<a href="javascript:void(0);" onclick="$.click_YX_page(' + (cv + 1) + ')" class="next">下一页</a>';
		return cr
	}
	function a5(cv, cq) {
		if (cq == 0) {
		...
		var cr = "";
		cr += (cv == 0) ? "" : '<a href="javascript:void(0);" onclick="$.click_passenger_page(' + (cv - 1) + ')" class="prev">上一页</a>';
		var cw = cv + 1;
		var cs = cq;
		var ct = 2;
		var cu = 5;
		...
		if (cs < cu) {
			for (var cp = 1; cp < cs + 1; cp++) {
				if (cw == cp) {
					cr += '<a href="javascript:void(0);" onclick="$.click_passenger_page(' + (cp - 1) + ')" class="on">' + (cp) + "</a>"
				} else {
					cr += '<a href="javascript:void(0);" onclick="$.click_passenger_page(' + (cp - 1) + ')">' + (cp) + "</a>"
				}
			}
		} else {
			for (var cp = cn; cp < co; cp++) {
		...
		cr += (cv == cq - 1) ? "" : '<a href="javascript:void(0);" onclick="$.click_passenger_page(' + (cv + 1) + ')" class="next">下一页</a>';
		return cr
	}
	function ch() {
		if ($("#sf2").is(":checked")) {
	...
	jQuery.extend({
		...
		init_ul4li : function() {
			...
			if (train_tour_flag == "gc" && "Y" == isDwTicketResign) {
				...
				for (var co = 2, cq = cr.length; co < cq; co++) {
					cr.eq(co).find("input").attr("disabled", "disabled");
					cr.eq(co).find("label").attr("for", "").attr("style", "color: rgb(153, 153, 153)")
				}
			}
			...
			if (passengerAll) {
				...
				if (cr != "" && cr != "输入乘客姓名") {
					...
					for (var cA = 0; cA < cD; cA++) {
						...
						if (cz.passenger_name.indexOf(cr) > -1 || (cz.first_letter && cz.first_letter.toUpperCase().indexOf(cr.toUpperCase()) > -1)) {
							cn.push(cz)
						}
					}
				} else {
					cn = passengerAll.slice(passengerPageSize * (cF), Math.min(passengerPageSize * (cF + 1), passengerAll.length))
				}
				var cy = cn.length;
				var cw = [];
				var cs = 0;
				for (var cA = 0; cA < cy; cA++) {
					...
					var cx = cE == "成人" ? cz.passenger_name : cz.passenger_name + "(" + cE + ")";
					cx = cx.substring(0, 9);
					if (cr != "" && cr != "输入乘客姓名") {
						if (cz.passenger_name.indexOf(cr) > -1 || (cz.first_letter && cz.first_letter.toUpperCase().indexOf(cr.toUpperCase()) > -1)) {
							cs++;
							if ($.pHasInSelected(cz)) {
								if (co) {
									var cu = co.indexOf("'");
									co = co.substring(0, cu + 1) + "cur " + co.substring(cu + 1)
								} else {
									co = "class='cur'"
								}
							}
							cw[cA] = "<li style='width:110px' " + co + " p_value='" + cz.passenger_name + "(" + cE + ")(" + cz.passenger_id_no + ")' name='" + cz.passenger_type + "' codeType='" + cz.passenger_id_type_code + "' flag='" + cz.total_times + "'>" + cx + "</li>"
						}
					} else {
						cs++;
						if ($.pHasInSelected(cz)) {
							if (co) {
								var cu = co.indexOf("'");
								co = co.substring(0, cu) + "cur " + co.substring(cu)
							} else {
								co = "class='cur'"
							}
						}
					...
					function() {
						...
						if (!$(this).hasClass("cur")) {
							if (cI < 6) {
								var cG = "";
								var cH = true;
								if (cK.indexOf("成人") > -1 || cK.indexOf("残疾军人、伤残人民警察") > -1) {
									cG = "<span name='" + cK + "' class='sel-box w80'><a href='javascript:' onclick='$.addChildPassenger(\"" + cK + "\");' style='position:static;background:none;width:auto;' title='您可点击此乘车人添加儿童票。'>" + cK
											+ "</a><a class='close' href='javascript:' onclick='$.removeSel(this,\"" + cK + "\",1)'></a></span>";
									$("#setion_postion").append(cG);
									$.checkedPasseanger(cK)
								} else {
									if (cK.indexOf("学生") > -1) {
										var cJ = $(this);
										if ($.checkSeatTypes()) {
											cG = "<span name='" + cK + "' class='sel-box w80'>" + cK + "<a class='close' href='javascript:' onclick='$.removeSel(this,\"" + cK + "\",1)'></a></span>";
											$("#setion_postion").append(cG);
											$.checkedPasseanger(cK)
										} else {
											...
											dhtmlx.createWin({
												...
												callback : function() {
													$(cJ).prop("checked", false)
												},
												okId : "goto_student_to_adult",
												okCallBack : function() {
													var cL = cK.replace(/学生/, "成人");
													cG = "<span name='" + cK + "' class='sel-box w80'>" + cL + "<a class='close' href='javascript:' onclick='$.removeSel(this,\"" + cK + "\",1)'></a></span>";
													$("#setion_postion").append(cG);
										...
										if (cK.indexOf("儿童") > -1) {
											cG = "<span name='" + cK + "' class='sel-box w80' title='如需修改旅客类型，请修改相应常用联系人信息。'>" + cK + "<a class='close' href='javascript:' onclick='$.removeSel(this,\"" + cK + "\",1)'></a></span>";
											$("#setion_postion").append(cG);
											$.checkedPasseanger(cK)
										} else {
											cG = "<span name='" + cK + "' class='sel-box w80'>" + cK + "<a class='close' href='javascript:' onclick='$.removeSel(this,\"" + cK + "\",1)'></a></span>";
							...
							$.each($("#setion_postion span"), function(cL, cM) {
								if (cK == $(cM).attr("name")) {
									$(cM).remove();
									$.removePasseanger(cK);
									return
				...
				success : function(co) {
					...
					if (co.status) {
						if (co.data.noLogin == "true") {
							...
							if (co.data.exMsg != "" && co.data.exMsg != null && co.data.exMsg != "null") {
								b(co.data.exMsg);
								return
							}
							$("#sel-buyer").show();
							...
							if (cr && cr.length > 0) {
								...
								for (var cp = 0; cp < cq; cp++) {
									if (!$.checkIsHas(passengerAll, cr[cp])) {
										passengerAll.push(cr[cp])
									}
								}
							}
		...
		checkIsHas : function(cp, cq) {
			...
			for (var cn = 0; cn < co; cn++) {
				if (cp[cn].passenger_name == cq.passenger_name && cp[cn].passenger_id_type_code == cq.passenger_id_type_code && cq.passenger_id_no == cp[cn].passenger_id_no) {
					return true
				}
			}
			return false
		...
		pHasInSelected : function(cq) {
			...
			if (co > 0) {
				for (var cn = 0; cn < co; cn++) {
					...
					if (cp.passenger_name == cq.passenger_name && cp.passenger_id_type_code == cq.passenger_id_type_code && cp.passenger_id_no == cq.passenger_id_no) {
						return true
					}
				}
			}
		...
		showYxTrain : function() {
			...
			cm(2);
			var cp = $("#fromStationText").val() + $("#toStationText").val() + $("#train_date").val();
			if (trainListForIE.length == 0 || yxTrainChange != cp) {
				...
				$.ajax({
					...
					success : function(cu) {
						if (cu.status) {
							if (cu.data == null || cu.data.length == 0) {
								cm(3);
								trainListForIE = [];
								return
							}
							if (cu.data.flag == 1) {
								cu.data = b2(cu.data.result, cu.data.map)
							}
							trainListForIE = [];
							...
							var cx = "";
							if (train_tour_flag != null && train_tour_flag == "fc") {
								var cA = "<strong>".concat($("#fromStationText").val()).concat(" --> ").concat($("#toStationText").val()).concat("（").concat(aD($("#back_train_date").val())).concat('）</strong>共计<strong id="trainum">').concat(cu.data.length).concat("</strong>个车次");
								if (bV(cu.data)) {
								...
								if ($("#auto_query").is(":checked")) {
									...
									cA = cA.concat(cz + "<input type='checkbox' class='check' id='filterTic' /><div id='filterTicDiv' style='display:inline'><strong><label for='filterTic' style='cursor: pointer;'>仅查看刷到车次</label></strong></div>")
								}
								...
								var cA = "<strong>".concat($("#fromStationText").val()).concat(" --> ").concat($("#toStationText").val()).concat("（").concat(aD($("#train_date").val())).concat('）</strong>共计<strong id="trainum">').concat(cu.data.length).concat("</strong>个车次");
								if (bV(cu.data)) {
									cs = "<p class='ad-gt' id='DW_SHOW_STR' data='1'>高铁动卧 夕发朝至 风雨无阻 省时省钱</p>"
								} else {
									if (hainan_limit_msg && ae(cq, cu.data)) {
	...
	function at() {
		...
		var cn = $("#fromStationNameTemplate").html().replace("<!--", "").replace("-->", "");
		$.templates({
			stationNameTableTemplate : cn
		});
		var cn = $("#toStationNameTemplate").html().replace("<!--", "").replace("-->", "");
		$.templates({
	...
	function bE(co) {
		...
		ax.clearAll();
		$(co).each(function() {
			cn.push([ this.queryLeftNewDTO.station_train_code, this.queryLeftNewDTO.station_train_code ])
		});
		ax.addOption(cn);
	...
	function aa(co, cn) {
		...
		$("#loginSubAsyn").click(function() {
			var cp = aS();
			if (if_show_pass_code_login == "Y" && !verifyRandCodeUAM($("#randCode")[0], cp)) {
				return
			}
			if (if_show_pass_code_login == "N" && typeof (cp) !== "boolean") {
				login_errorMsg(cp);
				return
			}
			$.ajax({
					...
					if (cq.result_code == 0) {
						...
						if (cq.result_message != null) {
							if (cq.result_message == "登录名不存在!") {
								...
								if (cq.result_message == "验证码不正确！" && cq.result_message != "登录名不存在!") {
									aP();
							...
							if (if_show_pass_code_login == "Y") {
								showError($("#randCode")[0], cq.result_message)
							} else {
	...
	function aV() {
		window.sucessCallback = function() {
			...
			if (isAsync == ticket_submit_order.request_flag.isAsync) {
				if (K.queryLeftNewDTO.train_no.indexOf(cn) > -1 && cp.indexOf(K.queryLeftNewDTO.from_station_telecode) > -1 && co.indexOf(K.queryLeftNewDTO.to_station_telecode) > -1) {
					dhtmlx.createWin({
						winId : "confirmG1234",
						closeWinId : [ "close_conifrmdialog_G1234", "cancel_dialog_G1234" ],
						okId : "goto_integration_G1234",
	...
	function aT() {
		...
		if (train_tour_flag != null && train_tour_flag != "" && train_tour_flag == "fc") {
			cv = jQuery.inArray($("#back_train_date").val().substring(5, 10), change_dates_arr)
		} else {
			cv = jQuery.inArray($("#train_date").val().substring(5, 10), change_dates_arr)
		}
		if (cv != "-1") {
			cv = cv + 1;
			cA = firstShow;
			co = endShow;
			if (parseInt(cv) >= parseInt(firstShow) && parseInt(cv) <= parseInt(endShow)) {
				if (isOther) {
			...
			$("#date_range>ul>li").removeClass("sel");
			if (cx) {
				$("#date_range>ul>li:nth-child(" + cA + ")").children("span:first").removeClass();
				$("#date_range>ul>li:nth-child(" + cA + ")").children("span:last").removeClass();
			...
			bR = $("#date_range>ul>li:nth-child(" + cv + ")").children("span:first-child").text()
		}
	}
	function bN() {
		$("#query_ticket").unbind("click");
	...
	String.prototype.toDate = function() {
		...
		for ( var co in cq) {
			if (new RegExp("(" + co + ")").test(style)) {
				cn[cq[co]] = this.substring(cp.indexOf(RegExp.$1), cp.indexOf(RegExp.$1) + RegExp.$1.length)
			}
		}
